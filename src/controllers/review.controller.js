...
const reviewController = {
    ...
    createOrUpdate: async (req, res) => {
        try {
            ...
            const existing = await Review.findOne({ user_id: req.user._id, course_id });
            let review;
            if (existing) {
                ...
                review = await Review.create({ user_id: req.user._id, course_id, rating: Number(rating), comment: String(comment || '').trim() });
            }
            await recalcCourseStats(course_id);
            const populated = await Review.findById(review._id).populate('user_id', 'name avatar');
            res.status(201).json({ success: true, message: existing ? 'Cập nhật đánh giá thành công' : 'Đánh giá thành công', data: { review: populated } });
    ...
    getByCourse: async (req, res) => {
        try {
            ...
            const [reviews, total] = await Promise.all([
                Review.find({ course_id: courseId, is_active: 1 }).populate('user_id', 'name avatar')
                    .sort({ created_at: -1 }).skip((page - 1) * limit).limit(parseInt(limit)),
            ...
            res.json({ success: true, data: { reviews, stats: { average_rating: agg[0] ? Math.round(agg[0].avg * 10) / 10 : 0, review_count: agg[0] ? agg[0].cnt : 0 }, pagination: { page: parseInt(page), limit: parseInt(limit), total, pages: Math.ceil(total / limit) } } });
        } catch (error) { res.status(500).json({ success: false, message: error.message }); }
    },
    ...
    getMyReview: async (req, res) => {
        try {
            ...
            if (courseId && mongoose.Types.ObjectId.isValid(courseId)) query.course_id = courseId;
            const reviews = await Review.find(query).populate('course_id', 'title').sort({ created_at: -1 });
